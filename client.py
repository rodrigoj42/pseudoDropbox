...
def local_changes(user):
    ...
    for f in fileList:
        f = f[len(userpath)+1:]
        try: files[f] = {'time':os.stat(userpath+'/'+f).st_mtime,'type':False}
        except: 
            try: files[f] = old_local[f]
            except: files[f] = {'time':0,'type':None}
    for f in dirList:
        f = f[len(userpath)+1:]
        try: files[f] = {'time':os.stat(userpath+'/'+f).st_mtime,'type':True}
        except: 
            try: files[f] = old_local[f]
            except: files[f] = {'time':0,'type':None}
    return files
...
def upload(user, psswd, item, isfolder): 
    ...
    userpath = './myDropbox/'+user+'/'
    #link = urllib.quote(item,'')
    #print link
    if not os.path.exists(userpath):
        os.makedirs(userpath)
    if not isfolder:
        files = {'file': open(userpath+item, 'rb')}
        requests.post(url+"/"+user+"/"+psswd+"/upload/file", files=files, data={'item':item})
    else:
        requests.post(url+"/"+user+"/"+psswd+"/upload/folder", data={'item':item})
    return
...
def main():
    ...
    lastlogin = urllib.urlopen(url+'/'+user+'/'+psswd+'/lastlogin').read()
    print lastlogin
    # main loop
    while True:
