# Cliente do Clone do Dropbox
import os, hashlib, urllib, pickle, requests
from time import sleep
from getpass import getpass
from flask import send_file

# configs
class Client():
    def __init__(self):
        self.url = 'http://127.0.0.1:5000' # servidor
        self.path = '.' # pasta atual
        self.t = 5 # segundos de espera
        self.user = ""
        self.psswd = ""
        self.old_local = {}
        self.old_server = {}

    def auth(self):
        '''Envia usuario e hash para o servidor a fim de descobrir se o usuario esta ou nao autenticado'''
        authorized = False
        try:
            print self.url+"/"+self.user+"/"+self.psswd+"/auth"
            authorized = eval(urllib.urlopen(self.url+"/"+self.user+"/"+self.psswd+"/auth").read())
        except:
            authorized = False
            print '\'except:'+str(authorized)+'\''
        if not authorized: print "Falha na autenticacao. \n"
        return authorized

    def server_changes(self):
        try:
            new = urllib.urlopen(self.url+"/"+self.user+"/"+self.psswd+"/list")
            new = new.read()
            userpath = './myDropbox/'+self.user+'/'
            with open(userpath+'new.p','wb') as new_file:
                new_file.write(new)
            new = pickle.load(open(userpath+'new.p','rb'))
            os.remove(userpath+'new.p')
        except:
            new = self.old_server
        return new

    def download(self, item, isfolder):
        if item == 'files.p':
            return
        print 'Baixando novo arquivo,', item
        try:
            userpath = './myDropbox/'+self.user+'/'
            if not os.path.exists(userpath):
                os.makedirs(userpath)
            if isfolder:
                os.makedirs(userpath+item)
            else:
                urllib.urlretrieve(url+"/"+self.user+"/"+self.psswd+"/download/"+item, filename=userpath+item)
        except:
            return 'Nao foi possivel baixar o arquivo'
        return


    def listFiles(self, dirname):
        fileList = []
        for (rootdir, dirnames, filenames) in os.walk(dirname):
            fileList.extend(filenames)
            for subdir in dirnames:
                subList = listFiles(self, rootdir+subdir)
                for item in subList:
                    fileList.append(subdir+'/'+item)
                fileList.append(subdir)
        return fileList
    def local_changes(self):
        '''Checa se existe alguma mudanca no diretorio sendo observado'''
        files = {}
        userpath = './myDropbox/'+self.user+'/'
        if not os.path.exists(userpath):
            os.makedirs(userpath)
        lista = self.listFiles(userpath)
        for f in lista:
            try: files[f] = {'time':os.stat(userpath+f).st_mtime,'type':os.path.isdir(userpath+f)}
            except: 
                try: files[f] = old_local[f]
                except: files[f] = {'time':0,'type':None}
        return files


    def upload(self, item, isfolder): 
        print 'Fazendo upload de novo item: ', item
        userpath = './myDropbox/'+self.user+'/'
        if not os.path.exists(userpath):
            os.makedirs(userpath)
        if not isfolder:
            files = {'file': open(userpath+item, 'rb')}
            requests.post(url+"/"+self.user+"/"+self.psswd+"/upload/"+item+"/file", files=files)
        else:
            requests.post(url+"/"+self.user+"/"+self.psswd+"/upload/"+item+"/folder")
        return

    def update(self, server, local):
        global old_server, old_local

        changes = {}
        files = server.keys() + local.keys()
        vals = server.values() + local.values()
        for index in xrange(len(files)):
            item = files[index]
            changes[item] = {'status':None,'type':None}
            changes[item]['type'] = vals[index]['type']
            if (item in local) and (item not in server):
                changes[item]['status'] = 'Upload'
            if (item in server) and (item not in local):
                changes[item]['status'] = 'Download'
            if (item in server) and (item in local):
                if (server[item]['time'] - local[item]['time']) > 60.0: 
                    print 'Conflito: "%s". Tratando arquivo mais recente como correto' % item 
                    changes[item]['status'] = 'Download'
                if (local[item]['time'] - server[item]['time']) > 60.0:
                    print 'Conflito: "%s". Tratando arquivo mais recente como correto' % item 
                    changes[item]['status'] = 'Upload'
            if (item not in server) and (item in self.old_server):
                changes[item]['status'] = 'Delete local'
            if (item not in local) and (item in self.old_local):
                changes[item]['status'] = 'Delete server'
        
        for key in changes:
            if key == 'new.p':
                continue
            f_type = changes[key]['type']
            if changes[key]['status'] == 'Delete server': self.delete_server(key)
            if changes[key]['status'] == 'Delete local': self.delete_local(key)
            if changes[key]['status'] == 'Download': self.download(key, f_type)
            if changes[key]['status'] == 'Upload': self.upload(key, f_type)

        self.old_server = server
        self.old_local = local 
        return

    def delete_server(self, item):
        urllib.urlopen(self.url+'/'+self.user+'/'+self.psswd+'/delete/'+item)
        print 'Removendo "%s" do servidor' % item
        return

    def delete_local(self, item):
        userpath = './myDropbox/'+self.user+'/'
        try: os.rmdir(userpath+item)
        except: 
            try: os.remove(userpath+item)
            except: print 'Erro ao deletar arquivo'
        print 'Removendo', item
        return

def main():
    # auth
    client = Client()
    logged = False
    while not logged:
        client.user = raw_input("Usuario: ")
        client.psswd = hashlib.sha512(getpass("Senha: ")).hexdigest()
        logged = client.auth()
    lastlogin = urllib.urlopen(client.url+'/'+client.user+'/'+client.psswd+'/lastlogin').read()
    print lastlogin
    # main loop
    while True:
        client.update(client.server_changes(), client.local_changes())
        sleep(client.t)

if __name__ == "__main__": 
    main()

