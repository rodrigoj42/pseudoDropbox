...
def auth(user, psswd):
    ...
    if user not in users:
        users[user] = {'password':psswd, 'acessos':0}
...
@app.route('/<user>/<psswd>/logincount')
def logincount(user, psswd):
    if auth(user, psswd) != 'True': return 'Falha na autenticacao.'
    users = pickle.load(open('./myDropboxServer/users.p','rb'))
    users[user]['acessos'] += 1
    if users[user]['acessos'] <= 1:
        plural = ''
    else:
        plural = 's'
    logincount = 'Voce teve %s acesso%s ate hoje.' % (users[user]['acessos'], plural)
    pickle.dump(users, open('./myDropboxServer/users.p','wb'))
    return logincount
